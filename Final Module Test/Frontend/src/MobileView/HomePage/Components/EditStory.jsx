          ...
          try {
            setTimeout(async () => {
              const response = await axios.post(
                "https://backend-73bf.onrender.com/story",
                item,
                {
                  headers: {
                    "content-type": "application/x-www-form-urlencoded",
  ...
  return (
    <div className="ADDSTORYY">
      ...
      <div className="changeslidesection">
        ...
        {status[0] === "error" && (
          <div className="error">
            Your form is incomplete. Checked, not working? Try logging in again.
          </div>
        )}
        {status[0] === "minbreach" && (
...
async function getStoryID() {
  try {
    let id = await axios.get("https://backend-73bf.onrender.com/storyid");
