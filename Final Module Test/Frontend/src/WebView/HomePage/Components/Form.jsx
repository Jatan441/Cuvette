...
export default function Form(props) {
  ...
  const handleSubmit = async (e, formData) => {
    e.preventDefault();
    if(props.isSignUp) {
      const response = await axios.post("https://swiptory-three.vercel.app/register", formData, {
        headers: {
    ...
    if(props.isLogIn) {
      const response = await axios.post("https://swiptory-three.vercel.app/login",formData , {
        headers: {
          "content-type": "application/x-www-form-urlencoded",
        },
      })
...
function signUp(formData, setFormData, handleSubmit, response) {
  return (
    <>
      <form
        action="https://swiptory-three.vercel.app/register"
        method="POST"
        onChange={(e) =>
...
function logIn(formData, setFormData, handleSubmit, response) {
  return (
    <>
      <form
        action="https://swiptory-three.vercel.app/login"
        method="post"
        onChange={(e) =>
          setFormData({ ...formData, [e.target.name]: e.target.value })
        }
